...
from Agent.chat_agent import more_speak
from Agent.login_agent import more_speak_login
from Agent.chat_agent_patient import more_speak_patient
from tool.sql_service import sql_tool_pool
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=origins,  # 允许所有来源（生产环境建议写具体域名）
...
def chat(args: ChatArgs):
    try:
        # 定义一个参数列表
        #question_list = [args.questions] if isinstance(args.questions, str) else args.questions
        question_list=[args.questions]
        # 定义一个会话id列表
        session_id_list=[args.userId]
        #判断你的问题的关键字
        if "邮箱" in args.questions:
            data = more_speak_patient(question_list,session_id_list)
        else:
            data =more_speak(question_list,session_id_list)

        # 定义一个消息对象
        d = {"code":200,"data":data,"message":"请求成功"}
        return  d
    except Exception as e:
        d = {"code": 500, "data":None, "message": "请求失败"}
        return d
